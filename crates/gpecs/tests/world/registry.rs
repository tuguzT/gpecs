...
fn three_items() {
    ...
    assert_eq!(world1.into_inner(), 1);
    assert_eq!(world2.into_inner(), 2);
    assert_eq!(world3.into_inner(), 3);
    assert!(worlds
        .world_ids()
...
fn overflow_items() {
    ...
    for idx in 0..u16::MAX - 1 {
        let world = worlds.spawn();
        assert_eq!(world.into_inner(), idx + 1);
        assert_eq!(worlds.len(), idx + 2);
    }

    let world = worlds.spawn();
    assert_eq!(world.into_inner(), u16::MAX);
    assert_eq!(worlds.len(), u16::MAX);

    let world = worlds.spawn();
    assert_eq!(worlds.len(), u16::MAX);
    assert_eq!(world.into_inner(), 0);
}

...
use gpecs_soa_erased::{
    ...
    soa::{
        field::FieldLayouts,
        traits::{Ptrs, Refs, SlicePtrs, Slices},
    },
};

use crate::{erased::ErasedArchetypeView, storage::ErasedArchetypeSoa};

type Inner<'ctx, T> = gpecs_sparse::iter::IterPtrs<'ctx, NoEpochEntity, T>;
    ...
    T: ErasedArchetypeSoa + ?Sized,
{
    ...
    #[inline]
    fn map_item(item: (*const NoEpochEntity, Ptrs<'ctx, T>)) -> (Entity, Refs<'ctx, 'a, T>) {
        let (entity, bundle) = item;
        let entity = unsafe { *entity.as_ref_unchecked() }.into();
        let bundle = unsafe { bundle.as_ref_unchecked() };
        (entity, bundle)
    }
}
...
impl<T> FusedIterator for Iter<'_, '_, T> where T: ErasedArchetypeSoa + ?Sized {}

    ...
    T: MutSliceItemPtr,
{
    ...
    #[inline]
    pub fn as_field_slice_ptr(&self) -> ErasedSlicePtr<CastConstPtr<T>> {
        let Self { ptr, .. } = self;
        ptr.cast_const()
    }
...
{
    #[inline]
    pub fn new(layout: Layout, buffer: &'a mut [U], len: usize) -> Result<Self, SlicePtrError> {
...
impl<'a, T> From<ErasedMutSlice<'a, T>> for ErasedSlice<'a, CastConstPtr<T>>
where
    T: MutSliceItemPtr,
{
    #[inline]

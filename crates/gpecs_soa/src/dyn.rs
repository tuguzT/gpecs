...
// TODO: add generic for alignment checks
#[derive(Debug, Clone)]
pub struct DynSoaContext {
...
#[derive(Debug)]
pub struct DynSoaSlicesMut<'a> {
    slices: Box<[DynFieldSliceRefMut<'a>]>,
}

unsafe impl Soa for DynSoa {
    // TODO: should respect alignment of the context
    type SizeAlign = ();

    type Context = DynSoaContext;
    ...
    type Ptrs = DynSoaPtrs;

    type MutPtrs = DynSoaMutPtrs;
    ...
    unsafe fn ptrs(
        ...
        let mut_ptrs: Box<[_]> = field_layouts
            .iter()
            .zip(offsets)
            ...
            .collect();
        assert_eq!(field_layouts.len(), mut_ptrs.len());
        DynSoaMutPtrs { ptrs: mut_ptrs }
    }

    fn ptrs_dangling(context: &Self::Context) -> Self::MutPtrs {
        let DynSoaContext { field_layouts } = context;

        let mut_ptrs = field_layouts
            .iter()
            .map(|field_layout| {
    ...
    type NonNullPtrs = DynSoaNonNullPtrs;
    ...
    type SlicePtrs = DynSoaSlicePtrs;

    type SliceMutPtrs = DynSoaSliceMutPtrs;

    ...
    P: MutSliceItemPtr,
{
    ...
    pub fn iter(
        &'a self,
    ) -> ErasedSoaSlicesIter<'a, D::OutputIter, CastConst<P>, BufferOffsetsFromLayout> {
        let Self { ptrs, .. } = self;
    ...
    pub fn iter_mut(
        &'a mut self,
    ) -> ErasedSoaMutSlicesIter<'a, D::OutputIter, P, BufferOffsetsFromLayout> {
        let Self { ptrs, .. } = self;
    ...
    P: MutSliceItemPtr,
{
    ...
    type IntoIter = ErasedSoaSlicesIter<'a, D::OutputIter, CastConst<P>, BufferOffsetsFromLayout>;
    ...
    P: MutSliceItemPtr,
{
    ...
    type IntoIter = ErasedSoaMutSlicesIter<'a, D::OutputIter, P, BufferOffsetsFromLayout>;
...
{
    type Item = ErasedMutSlice<'a, P>;
    type IntoIter = ErasedSoaMutSlicesIter<'a, D::IntoIter, P, BufferOffsetsFromLayout>;
...
pub struct ErasedSoaMutSlicesIter<'a, D, P, F>
where
    D: ?Sized,
    P: MutSliceItemPtr,
{
    phantom: PhantomData<&'a mut [P::Item]>,
    ptrs: ErasedSoaMutSlicePtrsIter<D, P, F>,
}

impl<D, P, F> ErasedSoaMutSlicesIter<'_, D, P, F>
where
    P: MutSliceItemPtr,
{
    #[inline]
    pub unsafe fn from_ptrs(ptrs: ErasedSoaMutSlicePtrsIter<D, P, F>) -> Self {
        let phantom = PhantomData;
        Self { phantom, ptrs }
    }
}
...
impl<'a, D, P, F> ErasedSoaMutSlicesIter<'_, D, P, F>
where
    D: FieldLayouts<'a> + ?Sized,
    P: MutSliceItemPtr,
...
{
    #[inline]
    pub fn iter(&'a self) -> ErasedSoaMutSlicesIter<'a, D::OutputIter, P, F> {
        let Self { ptrs, .. } = self;
...
impl<'a, D, P, F> IntoIterator for &'a ErasedSoaMutSlicesIter<'_, D, P, F>
where
    D: FieldLayouts<'a> + ?Sized,
    P: MutSliceItemPtr,
    F: BufferOffsetsFrom<D::OutputItem> + Clone,
{
    type Item = ErasedMutSlice<'a, P>;
    type IntoIter = ErasedSoaMutSlicesIter<'a, D::OutputIter, P, F>;
...
impl<D, P, F> Debug for ErasedSoaMutSlicesIter<'_, D, P, F>
where
...
impl<'a, D, P, F> Iterator for ErasedSoaMutSlicesIter<'a, D, P, F>
where
...
impl<D, P, F> ExactSizeIterator for ErasedSoaMutSlicesIter<'_, D, P, F>
where
...
impl<D, P, F> FusedIterator for ErasedSoaMutSlicesIter<'_, D, P, F>
where
...
impl<'a, D, P, F> FieldLayouts<'a> for ErasedSoaMutSlicesIter<'_, D, P, F>
where
    D: FieldLayouts<'a> + ?Sized,
    P: MutSliceItemPtr,
{
...
impl<D, P, F> CovariantFieldLayouts for ErasedSoaMutSlicesIter<'_, D, P, F>
where
    D: CovariantFieldLayouts + ?Sized,
    P: MutSliceItemPtr,
{

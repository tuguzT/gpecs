...
where
    ...
    V: RawSoa + ?Sized + 'c + 'a,
{
    inner: soa::slice::Iter<'c, 'a, DenseItem<K, V>>,
}
...
{
    #[inline]
    pub(super) fn from_inner(inner: soa::slice::Iter<'c, 'a, DenseItem<K, V>>) -> Self {
        Self { inner }
    }

    #[inline]
    pub(super) fn into_inner(self) -> soa::slice::Iter<'c, 'a, DenseItem<K, V>> {
        let Self { inner } = self;
        inner
    }
    ...
    V: Soa + ?Sized,
{
    ...
    #[inline]
    pub fn into_slices(self) -> (&'a [K], V::Slices<'c, 'a>) {
        let (_, keys, values) = self.into_slices_with_context();
        (keys, values)
    }

    #[inline]
    pub fn into_slices_with_context(self) -> (&'c V::Context, &'a [K], V::Slices<'c, 'a>) {
        let Self { inner } = self;

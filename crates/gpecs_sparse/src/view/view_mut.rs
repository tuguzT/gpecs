    ...
    S: SparseItem<Index = K::SparseIndex, Epoch = K::Epoch>,
{
    ...
    #[inline]
    pub fn into_raw_values_mut_with_context(
        self,
    ) -> (&'ctx V::Context, RawValuesMut<'ctx, K, V, P>) {
        let Self { dense, .. } = self;
    ...
    #[inline]
    pub fn into_raw_iter_mut_with_context(self) -> (&'ctx V::Context, RawIterMut<'ctx, K, V, P>) {
        let Self { dense, .. } = self;
